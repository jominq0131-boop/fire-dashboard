...
월별 합계는 해당 월에 입력한 대표 잔액만 합산합니다. 전월 차이와 비교 가능한 실선은 같은 계좌 ID 집합을 가진 연속 월에 적용합니다. 시각적 추세는 실제 관측점 사이를 모두 연결하며 누락 월/구성 변경은 점선 참고선으로 구별합니다. 개별 계좌에도 누락을 건너뛰는 점선이 있습니다. 선은 표시용 경로이며 중간 값을 보간·저장하거나 0으로 바꾸지 않습니다. 미기록 월의 툴팁/상세는 미기록으로 유지하고 부분 입력은 빈 원으로 표시합니다.
