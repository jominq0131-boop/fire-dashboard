...
현재 남아 있는 병합 완료 단기 브랜치는 위 설정을 켠 뒤 GitHub에서 정리해도 안전하다. 삭제는 복구 가능한 원격 작업이지만, 사용자가 실제로 정리를 요청하기 전에는 삭제하지 않는다.

## Next recommended milestone

**Milestone 4: IndexedDB 저장 기반과 계좌 등록 화면**을 권장한다.

범위는 저장소 인터페이스, IndexedDB 어댑터, 계좌 등록·수정·비활성화의 최소 UI, 계좌 검증 테스트다. 월별 현금흐름 입력과 FIRE 계산은 다음 Milestone으로 미룬다. 이 단계가 완료되면 대시보드 빈 상태를 실제 계좌 구조와 연결할 수 있다.
...
> `docs/project-continuity.md`와 `AGENTS.md`를 읽고, 현재 GitHub 상태를 확인한 뒤 Milestone 4의 작은 작업 단위와 첫 issue를 제안해 줘.
