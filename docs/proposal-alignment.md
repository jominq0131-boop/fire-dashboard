...
| FIRE               | 명시적 가정, 첫 도달,3개 비교, 자동 저장, 시나리오별 자산/목표 복합 차트 | 인출 이후 유지 가능성 모델     |
| JSON               | v1/v2/v3 가져오기, v3 내보내기, FIRE 계획 포함, 원자적 추가 복원         | 별도 대용량 복구 설계          |
| UI                 | 공통 다크 테마·브랜드 타입 스케일·일본어 최소12px·하단 앱 바·키보드 차트 | 폭넓은 다중 브라우저/접근성 QA |
