...
# Everyday recording / Issue #25 — 2026-09-04

- 월말을 잊으면 기록하기 어렵다는 사용자 피드백에 대응해 오늘 확인일/잔액 기록, 계좌별 마지막 확인액과 상태, 잔액 우선 입력, 계좌별 입력 초점을 구현했다. 현재 합계와 월별 대표 기록을 구분하고, 과거 빈 달을 추정하지 않는다.
- DB v3는 accountMonth 인덱스만 추가한다. 기존 v2 자료는 필드 변경 없이 보존하며 인덱스 생성 실패의 rollback 후 재시도까지 검증했다. JSON v1→v2는 날짜를 추정하지 않고 기존 필드를 보존한다. 새 기록은 실제 날짜/월 일치를 검증하며 기존 기록의 날짜 미상 상태를 유지한다.
- format/lint/typecheck, 단위98개, Pages build, Chromium30개(45.4초), diff 검사 통과. 데스크톱/390px 화면과 모바일 가로 넘침을 확인했다. 첫 회귀 실행은 구버전2를 직접 열던 기존 테스트 준비 코드로 실패했으며 통과로 계산하지 않았다. 현재3/미래4로 테스트 기준을 옮기되 명시적 v1/v2 마이그레이션 검증을 유지했다. 이후 전체 재실행이 정상 종료했다.
- 합성 데이터/격리 컨텍스트/기존 브라우저/작업자1개를 사용했고 실제 사용자 금융 자료·프로필·백업은 읽거나 포함하지 않았다. 새 의존성/외부 서비스는 없다. 관련 진행/설계/모델/자원/검증/계층 문서를 갱신했다.
- 한 계좌당 월1개 대표값 정책은 유지하며 일별 이력은 아니다. 실제 금융기관 자동 수집/동기화/FIRE는 후속이다. 로컬 구현·검증 완료이며 PR CI/병합/배포 결과는 Issue #25 연결 PR에 확인 후 기록한다.

# Milestone 7 / Issue #23 — 2026-09-04

- 사용자 요청으로 최신 기록 월의 전체 자산을 자동 표시하고, 12개월 그래프/표와 월별 상세 이동을 구현했다. 기존 선택 월 지표는 입력 화면에 유지해 전체 개요와 분리했다. 미래 잔액은 현재 합계에서 제외하고, 누락/부분 입력과 비교 불가능한 월을 명시했다.
- JSON v1 백업 다운로드, 크기/형식/참조 검증, 미리보기/취소, 동일 기록 건너뛰기와 추가 방식 원자적 복원을 구현했다. 기존 DB v2와 의존성은 유지했다. 정규화는 로케일에 의존하지 않는 정렬이며 구/미래 JSON 버전은 거부한다.
- 로컬 format/lint/typecheck, 단위93개, root/Pages build, Chromium28개(40.6초) 통과. 최종 결정론적 정렬/그래프 빈 상태/스타일 조정 후 단위93개와 관련 Chromium3개(6.9초)를 재검증했다. 비동기 제약 오류로 앞선 계좌 추가까지 rollback하고 기존 데이터가 그대로임을 확인했다. 실제 다운로드 파일을 별도 빈 브라우저 컨텍스트에 복원했다. 데스크톱/390px 스크린샷을 확인했고 모바일 가로 넘침 검사를 통과했다. 기존 테스트는 삭제·약화하지 않았다.
- 합성 데이터와 격리 브라우저, 기존 설치, 작업자1개만 사용했다. 실제 사용자 데이터/프로필/백업은 읽거나 커밋하지 않았다.
- README/연속성/계산·JSON 계약/설계/자원·검증/계층 문서와 milestone-7-plan.md를 갱신했다. 운영 정책과 AGENTS는 변경하지 않았다.
- 로컬 검증 완료, PR CI/병합/공개 배포 결과는 Issue #23 연결 PR에 확인 후 기록한다.32 MiB 초과 백업, 덮어쓰기 복원, 자동 동기화, FIRE/PWA는 미구현이다. 화면 밝기의 전체 재검토는 보류한다.

# Milestone 6 / Issue #21 — 2026-09-04

- 사용자 요청에 따라 문서와 최신 main 95eddb4, 열린 issue/PR 부재, main CI/Pages 성공을 확인하고 feat/monthly-dashboard-metrics에서 구현했다.
- 저장된 월별 잔액 합계·입력 커버리지·수입/소비/투자·소비 후/투자 후 잉여를 연결했다. 미입력/0, 휴지 계좌, 음수 및 안전 정수 초과를 구분한다. 미저장 입력/실패 쓰기는 요약을 바꾸지 않으며 월 전환/읽기 실패는 이전 요약을 숨긴다.
- 새 의존성, DB v2 스키마/마이그레이션, 저장 구조, 외부 서비스 변경은 없다. 실제 데이터/백업을 읽거나 포함하지 않았다. 문서 영향은 진행/계산 계약/계층/검증에 반영했다. 자원 정책과 인프라 계약은 불변이다.
- 로컬 format/lint/typecheck, 단위 75개, build, Chromium 25개(27.8초), diff 검사 통과. 모바일 390px 스크린샷 확인 후 설명문 간격을 수정하고 관련 Chromium 2개(3.9초)와 format을 재검증했다. 기존 테스트는 유지했다. 사용자 브라우저 프로필 없이 격리 컨텍스트/기존 브라우저/작업자 1개를 사용했다.
- 현재 로컬 검증 완료. PR 최종 CI, main 병합과 실제 Pages 확인은 Issue #21 연결 PR에 실행 후 기록한다. JSON 백업/복원, 차트/히스토리, FIRE/PWA는 미완료다. UI 밝기는 기능 완성 뒤 재검토한다.
...
## 2026-09-04 — UI/UX refresh / Issue #19

- 사용자 요청으로 정보 위계·여백·글자·폼·탐색을 정리했다. 데스크톱 사이드바/2열과 모바일 1열, 월 선택 빈 상태, 펼쳐보는 계좌 저장 설명을 제공한다.
